...
import { getProductsDeactivate, putProducts } from "../../../shared/services";
...
export function ModalDeactivateProduto({
  open,
  loadProducts,
  toggleModal,
  getInsumoNome,
  ...
  const columns: GridColDef<ProdutoDataRow>[] = [
    ...
    },
    {
      ...
      width: 100,
      headerClassName: "gridHeader--header",
      renderCell: (params) => <span>{getCategoriaNome(params.value)}</span>,
    },
    {
      field: "idInsumo",
      headerName: "Insumo",
      ...
      width: 110,
      headerClassName: "gridHeader--header",
      renderCell: (params) => <span>{getInsumoNome(params.value)}</span>,
    },
    {
      field: "largura",
      headerName: "Largura",
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <div>
          <IconButton
            onClick={() => row.id !== undefined && handleActivate(row)}
          >
            <AddCircleIcon />
          </IconButton>
        </div>
